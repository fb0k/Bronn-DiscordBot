...
class ErrorHandler(Cog, command_attrs=dict(hidden=True), emoji=Emojis.pycord):
    ...
    async def on_command_error(self, ctx: Context, e: errors.CommandError) -> None:
        ...
        debug_message = f"Command {command} invoked by {ctx.message.author} with error " f"{e.__class__.__name__}: {e}"
        ...
        if isinstance(e, errors.CommandNotFound) and not getattr(ctx, "invoked_from_error_handler", False):
            ...
            if isinstance(e.original, LockedResourceError):
    ...
    async def handle_unexpected_error(ctx: Context, e: errors.CommandError) -> None:
        ...
        with push_scope() as scope:
            ...
            if ctx.guild is not None:
                scope.set_extra(
                    "jump_to", f"https://discordapp.com/channels/{ctx.guild.id}/{ctx.channel.id}/{ctx.message.id}"
                )
